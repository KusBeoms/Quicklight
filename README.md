...
| 기능 | 설명 |
|---|---|
| 접두 기호 없이 최적의 결과 | 앱, 파일, 설정, 명령, 계산기, URL, 경로, 웹 검색을 한 번에 찾아 가장 알맞은 순서로 보여 줌. 자주 고른 결과는 비슷하게 검색하면 위로 올라옴 |
| 오타 보정 | `fuison`, `fuson`, `gusion` 처럼 틀려도 가장 가까운 이름(Fusion)을 찾음. 옆 키를 잘못 누른 오타는 더 가깝게 봄. 네 글자 이상의 영어 검색어에 적용 |
| 한글 ↔ 영어 | `애플뮤직` 으로 Apple Music, `크롬` 으로 Chrome, `melon` 으로 멜론처럼 소리가 같으면 다른 글자로 쳐도 찾음. Windows 기본 앱은 한글 이름(`파일탐색기`, `명령 프롬프트`, `제어판`)과 영어 이름(`file explorer`, `command prompt`) 둘 다로 찾음 |
| 파일, 폴더 검색 | Everything 색인으로 즉시 검색. 같은 이름의 파일이나 앱이 많아도 이름이 맞는 폴더가 결과에 남음 |
| 앱 하나에 한 줄 | 시작 메뉴와 바탕 화면의 바로 가기, 실행 파일, 업데이터나 오류 보고 같은 딸린 도구, 다운로드 폴더의 설치 파일, 제거 프로그램, 스토어 앱을 앱 하나로 묶어 한 줄로 보여 줌. Enter는 바로 가기 → 실행 파일 → 설치 파일 중 남아 있는 것을 실행. 설치 파일 실행과 제거는 우클릭 메뉴에 있음 |
| Spotlight처럼 간결한 UI | 알약 모양 검색창, 그 아래에 부드럽게 늘어나는 둥근 결과 카드. 흐린 배경, "최상위 결과"와 종류별 구역, 열 때 흐림에서 초점이 잡히고 닫을 때 부드럽게 사라지는 애니메이션, 은은하게 깜빡이는 커서, 시스템 밝은/어두운 테마 |
| 모든 앱 보기 | `앱`, `app`, `apps`, `application`, `애플리케이션`, `응용프로그램`, `프로그램` 을 입력하면 설치된 앱 전체를 이름순 그리드로 표시 (A–Z, ㄱ–ㅎ, # 구역. 제거 프로그램과 도움말 링크는 뺌). 그리드에서 글자를 치면 검색창 대신 그 앱으로 바로 이동 (`c` 는 C 구역 첫 앱, `clau` 는 Claude. 친 글자는 검색창 오른쪽에 보이고 1초 멈추면 지워짐, 최대 10자. Backspace는 친 글자부터 지우고, 다 지우면 검색어를 고쳐 그리드에서 나감) |
| 우클릭 메뉴 | 열기, 관리자 권한으로 실행, 파일 탐색기에서 열기, 경로 복사, 속성. 앱에는 설치 파일 실행, 제거도 |
| 미리보기 | 파일, 폴더, 앱을 고른 채 Space를 1초 누르고 있으면 미리보기. 미리보기가 되는 줄은 오른쪽에 › 표시. 누르는 동안 줄이 당겨지다가 튀어나오듯 열림. 이미지, 텍스트와 코드, 폴더 내용, 그 밖에는 탐색기 썸네일(PDF, 동영상). 앱은 게시자와 버전 아래에 묶인 바로 가기, 실행 파일, 도구, 설치 파일, 제거 프로그램을 앱처럼 한 줄씩 보여 주고, ↑ ↓ 로 골라 Enter로 실행 (제거는 Enter 두 번). 모든 앱 그리드에서도 됨 |
| 클립보드 기록 | `clip`, `클립보드`, `ㅋㄹ` (+ 거를 단어). Win+V와 같은 Windows 클립보드 기록. 모든 앱 화면처럼 넓은 창에 글 앞부분이나 이미지를 큰 카드로 보여 줌. Enter는 원래 창에 붙여넣기, Space 길게는 전체 글이나 원본 크기 이미지 보기, Del은 기록에서 삭제 |
...
| 환전 | `100달러`, `$100`, `5만원 엔`, `오백달러`, `천오백엔`, `100 usd to eur`, `100달러를 유로로`, `100달러는 몇 원?`. 대상을 안 쓰면 Windows 지역 설정의 통화(한국이면 원화)로, 이미 그 통화면 달러로 바꾸고 다른 주요 통화도 함께 보여 줌 |
...
| 진행 표시줄 | 업데이트 다운로드, 앱 목록과 파일 색인을 검색창 바로 아래 얇은 막대로 표시. 양을 알 수 있으면 백분율, 모르면 흐르는 막대 |
...
- **수동**: 런처에 `update` 또는 `업데이트` 를 치면 곧바로 확인합니다. 새 버전이 있으면 Enter 없이 바로 설치를 시작하고(진행률은 검색창 아래 막대, Esc로 취소), 없으면 "최신 버전입니다"라고 알려 줍니다. Enter를 누르면 다시 확인합니다.
...
| ↑ ↓ (검색창이 비었을 때) | 최근 검색어 (터미널처럼 ↑ 은 이전, ↓ 은 다음. 최근 10개). 불러온 검색어는 전체 선택된 상태로 들어오고, 선택을 풀면 ↑ ↓ 는 다시 결과 이동. Ctrl+A나 드래그로 글자 전체를 다시 선택하면 최근 검색어 이동으로 돌아감 (일부만 선택하면 아님) |
| ↑ ↓, PgUp PgDn, 마우스 휠 | 결과 이동 (앱 그리드에서는 ← → 도. 앱 그리드에서 휠은 스크롤) |
| Enter, 휠 클릭 | 열기, 실행 (계산 결과는 클립보드로 복사) |
| Ctrl+Enter | 탐색기에서 파일 위치 열기 |
| Ctrl+Shift+Enter | 관리자 권한으로 실행 |
| Alt+Enter | 속성 |
| Space 길게 (1초) | 파일, 폴더, 앱, 클립보드 항목 미리보기. Space나 Esc로 닫고, ↑ ↓ 로 다른 항목 미리보기 (앱은 묶인 항목 선택, Enter로 실행). 마우스 휠도 ↑ ↓ 처럼 (텍스트는 스크롤). 짧게 누르면 그냥 띄어쓰기 |
| Del | 클립보드 기록에서 삭제 |
| Ctrl+Shift+C | 경로 복사 |
| 우클릭 | 열기, 관리자 권한으로 실행, 파일 탐색기에서 열기, 경로 복사, 속성 |
| Alt+. | Quicklight 설정 창 열기 |
| Alt+Shift+. | Everything 설정 창 열기 (색인할 폴더와 드라이브, 제외 목록 등) |
...
검색창에서 Alt+. 이나 트레이 메뉴 "설정…"으로 설정 창을 엽니다. 저장하면 바로 적용됩니다.

설정은 `%APPDATA%\Quicklight\settings.json` 에 저장됩니다. 설정 창 아래의 "설정 파일 열기"로 파일을 직접 고칠 수 있고, 고친 뒤 트레이 메뉴 "설정 다시 불러오기"로 적용합니다. 아래 표의 설정 중 `updateRepository`, `bundledEverything`, `everythingSetup` 은 파일에서만 바꿀 수 있습니다.
...
| `advancedSearch` | `true` | 아래 결과 수와 경로 규칙(`maxResults`, `maxFileResults`, `maxFolderResults`, `demotedPaths`, `excludedPaths`)을 씀. `false` 면 기본값으로 검색하고 적어 둔 값은 그대로 남음 (설정 창의 "상세 설정") |
...
런처는 열 때마다 빈 검색창으로 시작합니다. 결과를 실행하거나 복사한 검색어만 최근 10개까지 `%APPDATA%\Quicklight\recent.json` 에 남아, 빈 검색창에서 ↑ 로 다시 불러올 수 있습니다.

사용 기록은 `%APPDATA%\Quicklight\history.json` 에 있습니다. 여기에는 검색어를 저장하지 않습니다. 고른 결과와 시각, 몇 글자를 쳤는지, 검색어가 결과 이름과 얼마나 맞았는지만 남기고, 이것으로 비슷하게 검색할 때 그 결과를 위로 올립니다. 이전 버전의 기록에 남아 있던 검색어는 처음 실행할 때 지웁니다. 앱 목록은 `%LOCALAPPDATA%\Quicklight\apps.db`, 로그는 `%LOCALAPPDATA%\Quicklight\quicklight.log` 에 있습니다.
